...
describe("DateUtils:dateIsToday", function()
    ...
    defect("a timestamp exactly one year old counts as today", function()
        -- yday is compared without the year, so 17 June 2025 and 17 June 2026 are
        -- the same day as far as this function is concerned. It only bites a
        -- character with more than a year of history, which retentionDays = 0
        -- (the default) allows.
...
describe("DateUtils:dateIsInCurrentMonth", function()
    ...
    defect("the same month a year ago counts as this month", function()
        -- month is compared without the year, so June 2025 is "this month" in
        -- June 2026. Same root cause as the dateIsToday defect above.
...
describe("DateUtils:dateInRangeTillToday", function()
    ...
    defect("the range drops loot when it spans New Year", function()
        -- The upper bound is `yday <= today.yday`, comparing day-of-year without
        -- the year. On 2 January the previous Wednesday is in December, whose yday
        -- is ~360 - greater than today's 2 - so every entry from last year is
        -- excluded and the Wednesday-to-Wednesday range looks nearly empty.
        wow.freeze({ year = 2026, month = 1, day = 2, hour = 12 })

        local from = DU:getLastWed(DU:getToday().wday)
        local lastYear = os.time({ year = 2025, month = 12, day = 30, hour = 12 })
